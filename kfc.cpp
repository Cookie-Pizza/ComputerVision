...
void KCircleHough::Init(void *pInfo){
    ...
    _arVotes.Create(_oInfo.nNumGridCy, _oInfo.nNumGridCx);

    for(int i = 0; i<_oInfo.nNumGridCy ; i++){
        for(int j = 0; j<_oInfo.nNumGridCx ; j++){
            _arVotes[i][j] = new double[_oInfo.nNumGridR];
            memset(_arVotes[i][j],0,_oInfo.nNumGridR*sizeof(double));
        }
    }

    _dSx = (_oInfo.nNumGridCx==1 ? 0 : (_oInfo.nRight - _oInfo.nLeft)/(double)(_oInfo.nNumGridCx-1));
    _dSx = (_oInfo.nNumGridCy==1 ? 0 : (_oInfo.nBottom - _oInfo.nTop)/(double)(_oInfo.nNumGridCy-1));
    _dSr = (_oInfo.nNumGridR==1 ? 0 : (_oInfo.nMaxR - _oInfo.nMinR)/(double)(_oInfo.nNumGridR-1));
}
